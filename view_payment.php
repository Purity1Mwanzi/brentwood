...
<?php 
$tenants =$db->query("SELECT t.*,concat(t.lastname,', ',t.firstname,' ',t.middlename) as name,h.house_no,h.price FROM tenants t inner join houses h on h.id = t.house_id where t.id = {$_GET['id']} ");
...
$paid = $db->query("SELECT SUM(amount) as paid FROM payments where tenant_id =".$_GET['id']);
$last_payment = $db->query("SELECT * FROM payments where tenant_id =".$_GET['id']." order by unix_timestamp(date_created) desc limit 1");
		...
		<div class="row">
			...
			<div class="col-md-8">
				...
				<table class="table table-condensed table-striped">
					...
					<tbody>
						<?php 
						$payments = $db->query("SELECT * FROM payments where tenant_id = $id");
