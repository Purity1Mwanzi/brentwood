...
if(isset($_GET['id'])){
$qry = $db->query("SELECT * FROM tenants where id= ".$_GET['id']);
	...
	<form action="" id="manage-tenant">
		...
		<div class="form-group row">
			<div class="col-md-4">
				...
				<input type="email" id="confirm-email" class="form-control" name="email"  value="<?php echo isset($email) ? $email :'' ?>" required>
			</div>
			<div class="col-md-4">
				<label for="" class="control-label">Contact #</label>
				<input type="text" id="contact-number" class="form-control" name="contact"  value="<?php echo isset($contact) ? $contact :'' ?>" required>
			</div>
		...
		<div class="form-group row">
			<div class="col-md-4">
				...
				<select name="house_id" id="" class="custom-select select2">
					<option value=""></option>
					<?php 
					$house = $db->query("SELECT * FROM houses where id not in (SELECT house_id from tenants where status = 1) ".(isset($house_id)? " or id = $house_id": "" )." ");
...
<script>
	...
	$('#manage-tenant').submit(function(e){
		...
		var regexContact = /^\d{10}$/;
				...
				if(!regexEmail.test($("#confirm-email").val())){
            swal('email is invalid')
        
  
          }
		  else if(!regexContact.test($("#contact-number").val())){
			swal('contact is invalid')
		  }
		  else{
// start_load()
$('#msg').html('')
