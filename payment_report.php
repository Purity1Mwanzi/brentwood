						...
						<div class="row">
							<table class="table table-bordered">
								...
								<tbody>
									...
									$tamount = 0;
									if($_SESSION['ut'] == 1){
										$payments  = $db->query("SELECT p.*,concat(t.lastname,', ',t.firstname,' ',t.middlename) as name,h.house_no FROM payments p inner join tenants t on t.id = p.tenant_id inner join houses h on h.id = t.house_id where date_format(p.date_created,'%Y-%m') = '$month_of' order by unix_timestamp(date_created)  asc");

									}
									else{
										$payments  = $db->query("SELECT p.*,concat(t.lastname,', ',t.firstname,' ',t.middlename) as name,h.house_no FROM payments p inner join tenants t on t.id = p.tenant_id inner join houses h on h.id = t.house_id where date_format(p.date_created,'%Y-%m') = '$month_of' and p.tenant_id = {$_SESSION['tenant_id']} order by unix_timestamp(date_created)  asc");
